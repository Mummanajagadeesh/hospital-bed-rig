...
class EyeBlinkDetector(QThread):
        ...
        while self.running:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    left_eye_ratio = self.calculate_eye_ratio(face_landmarks.landmark, [33, 160, 159, 158, 153, 144, 145, 133])
                    right_eye_ratio = self.calculate_eye_ratio(face_landmarks.landmark, [362, 385, 387, 386, 374, 380, 381, 263])
    ...
    def calculate_eye_ratio(landmarks, indices):
        ...
        horizontal_distance = ((landmarks[indices[0]].x - landmarks[indices[4]].x) ** 2 +
                               (landmarks[indices[0]].y - landmarks[indices[4]].y) ** 2) ** 0.5

        vertical_distance_1 = ((landmarks[indices[1]].x - landmarks[indices[5]].x) ** 2 +
                               (landmarks[indices[1]].y - landmarks[indices[5]].y) ** 2) ** 0.5
        vertical_distance_2 = ((landmarks[indices[2]].x - landmarks[indices[6]].x) ** 2 +
                               (landmarks[indices[2]].y - landmarks[indices[6]].y) ** 2) ** 0.5
...
class EyeControlGUI(QMainWindow):
    ...
    def switch_screen(self, screen_name):
        ...
        if screen_name == "sideways":
            ...
            self.option_list = [self.angle_30_button, self.angle_45_button, self.angle_60_button]
